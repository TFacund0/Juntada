...
- **`useAppSession.ts`** — qué juego/modo está elegido y el flujo de grupo;
  ...
  transición; eso vive en `useAppDialogs`/`useHeaderUI` y `useAppShell`
  respectivamente. Lee el esquema de URL desde `../routing/appRoutes.ts`
  (`routeInitFromMatches`) para derivar el estado inicial en una carga
  directa o un refresh.
- **`useAppDialogs.ts`** — estado de las confirmaciones reales de
  "volver"/"salir", también extraído de `useAppNavigation.ts`. Los toggles
  del header (profile menu / rules) NO viven acá, ver `useHeaderUI.ts`.
