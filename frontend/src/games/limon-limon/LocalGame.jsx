...
function nextTurnId(players, currentId) {
...
function Ranking({ players, piles }) {
  const ranked = players
    .map(p => ({ ...p, count: (piles[p.id] || []).length }))
...
export function LocalGame() {
  ...
  const [endVotes, setEndVotes] = useState([]); // ids que votaron terminar antes
  ...
  const toggleEndVote = (id) => setEndVotes(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  ...
  if (phase === "setup") return (
    <div>
      ...
      <Btn onClick={startGame} disabled={players.length < MIN_PLAYERS}>Empezar a jugar</Btn>
  ...
  if (phase === "play") return (
    <div>
      ...
      {current && (
        <div style={{ ...S.cardHighlight, marginTop: 14 }}>
          {getDescription(descriptions, current) && (
            <p style={{ textAlign: "center", fontSize: 13, color: "#b8b0d4", margin: "0 0 12px" }}>"{getDescription(descriptions, current)}"</p>
          )}
          <span style={{ ...S.label, textAlign: "center", display: "block" }}>¿Quién se la queda?</span>
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
            {players.map(p => (
              <button key={p.id} onClick={() => assign(p.id)} style={{ ...S.btn("secondary"), display: "flex", alignItems: "center", gap: 10, justifyContent: "flex-start", padding: "10px 14px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14, marginBottom: 14 }}>
        <button onClick={() => setShowRanking(v => !v)} style={{ ...S.btn("secondary"), flex: 1, fontSize: 13 }}>
          {showRanking ? "Ocultar puntaje" : "Ver puntaje"}
        </button>
        <button onClick={() => setAddingPlayer(v => !v)} style={{ ...S.btn("secondary"), flex: 1, fontSize: 13 }}>
          + Sumar jugador
        </button>
        <button onClick={() => setVotingEnd(v => !v)} style={{ ...S.btn(endVotes.length > 0 ? "danger" : "secondary"), flex: 1, fontSize: 13 }}>
      ...
      {addingPlayer && (
        <div style={S.card}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input style={{ ...S.input, flex: 1 }} placeholder="Nombre" value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => { if (e.key === "Enter") addPlayer(); }} />
            <Btn variant="secondary" onClick={addPlayer} style={{ width: "auto", padding: "11px 18px" }}>Sumar</Btn>
          </div>
          {nameError && <p style={{ fontSize: 12, color: "#F09595", marginTop: 8 }}>{nameError}</p>}
        </div>
      )}
      ...
      {votingEnd && (
        <div style={S.card}>
          <span style={S.label}>Votar terminar antes ({endVotes.length}/{endThreshold})</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {players.map(p => {
              ...
              return (
                <button key={p.id} onClick={() => toggleEndVote(p.id)} style={{ ...S.btn(voted ? "danger" : "secondary"), display: "flex", alignItems: "center", gap: 10, justifyContent: "flex-start", padding: "10px 14px" }}>
                  <Avatar name={p.name} size={26} />
                  <span>{p.name}</span>
      ...
      <Btn variant="secondary" onClick={() => setPhase("setup")} style={{ marginTop: 4 }}>Abandonar partida</Btn>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
        ...
        <Btn variant="secondary" onClick={() => setPhase("setup")}>Volver a jugadores</Btn>
      </div>
    </div>
  );
}
