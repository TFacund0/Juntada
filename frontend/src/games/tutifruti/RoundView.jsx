...
function WritingPhase({ room, me, myPlayer, myRole, isHost, send }) {
  const round = room.round;
    ...
    <div>
      ...
      {round.endMode === "timer" && (
        myPlayer?.ready ? (
          ...
          <Btn variant="success" onClick={() => send({ type: "player_ready" })}>Ya terminé</Btn>
        )
      )}
      {round.endMode === "timer" && (
        <p style={{ ...S.muted, textAlign: "center", marginTop: 8 }}>{readyCount} de {onlinePlayers.length} jugadores ya terminaron</p>
      )}
...
function ReviewPhase({ room, me, send }) {
  ...
  return (
    <div>
      ...
      {round.categories.map(cat => {
        ...
        return (
          <div key={cat.id} style={S.card}>
            <span style={S.label}>{cat.icon ? `${cat.icon} ` : ""}{cat.label}</span>
            {entries.map(({ playerId, word }) => {
              ...
              return (
                <div key={playerId} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(127,119,221,0.08)" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 15, fontWeight: 600, wordBreak: "break-word", overflowWrap: "anywhere" }}>{word}</p>
                    {wrongLetter && <span style={{ fontSize: 11, color: "#F09595", fontWeight: 700 }}>✗ no empieza con "{round.letter}" — no cuenta</span>}
                  </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 2, width: 46, justifyContent: "flex-end", flexShrink: 0 }}>
                    {Object.values(marksForWord).map((valid, i) => (
                      <span key={i} style={{ fontSize: 11, color: valid ? "#5DCAA5" : "#F09595" }}>{valid ? "✓" : "✗"}</span>
                    ))}
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => send({ type: "mark_word", targetPlayerId: playerId, categoryId: cat.id, valid: true })}
                    ...
                    <button onClick={() => send({ type: "mark_word", targetPlayerId: playerId, categoryId: cat.id, valid: false })}
      ...
      {iConfirmed ? (
        <div style={{ ...S.card, textAlign: "center" }}>
          <p style={{ color: "#5DCAA5", margin: 0 }}>Confirmaste los puntajes — esperando a los demás ({confirmedCount}/{online.length})</p>
        </div>
      ) : (
        <Btn variant="success" onClick={() => send({ type: "confirm_review" })}>Confirmar puntajes ({confirmedCount}/{online.length})</Btn>
      )}
    </div>
  );
}
...
function ResultPhase({ room, isHost, send }) {
  ...
  const standings = [...room.players]
    .map(p => ({ ...p, score: room.config.score[p.id] || 0, roundPts: round.pointsByPlayer[p.id] || 0 }))
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        ...
        {round.categories.map(cat => {
          const entries = room.players
            .map(p => (round.breakdown[p.id] || {})[cat.id])
            .filter(b => b && b.word);
          if (entries.length === 0) return null;
          return (
            <div key={cat.id} style={{ marginBottom: 12 }}>
              <span style={{ fontSize: 12, color: "#7F77DD", fontWeight: 700 }}>{cat.icon ? `${cat.icon} ` : ""}{cat.label}</span>
              {entries.map((b, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, fontSize: 13, padding: "4px 0", color: "#b8b0d4" }}>
