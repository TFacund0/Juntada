...
export function RoundView({ room, me, myPlayer, isHost, send }) {
  const opponent = room.players.find(p => p.id !== me.playerId);
  const round = room.round;
  const score = room.config.score || {};
  ...
  const Scoreboard = (
    <div style={{ ...S.card, display: "flex", justifyContent: "space-around", textAlign: "center" }}>
      <div>
        <p style={{ margin: 0, fontWeight: 700, fontSize: 14 }}>{myPlayer?.name}{myMark ? ` (${myMark})` : ""}</p>
        <p style={{ margin: "2px 0 0", fontSize: 22, fontWeight: 800, color: "#AFA9EC" }}>{score[me.playerId] || 0}</p>
      </div>
      <div>
        <p style={{ margin: 0, fontSize: 12, color: "#6b6490" }}>Empates</p>
        <p style={{ margin: "2px 0 0", fontSize: 22, fontWeight: 800, color: "#9089c0" }}>{room.config.draws || 0}</p>
      </div>
      <div>
        <p style={{ margin: 0, fontWeight: 700, fontSize: 14 }}>{opponent?.name}{round?.marks?.[opponent?.id] ? ` (${round.marks[opponent.id]})` : ""}</p>
  ...
  const ResetScoreControl = (
    <div style={{ marginTop: 16 }}>
      {iVotedReset ? (
        ...
        <Btn variant="danger" onClick={requestResetScore} style={{ fontSize: 13 }}>{opponent?.name} pidió reiniciar el marcador — confirmar</Btn>
      ) : (
        <Btn variant="default" onClick={requestResetScore} style={{ fontSize: 13 }}>Reiniciar marcador</Btn>
      )}
    </div>
  );
  ...
  if (room.phase === "result" && round) {
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: 16 }}>
          {myPlayer?.ready ? (
            <div style={{ ...S.card, textAlign: "center" }}><p style={{ color: "#5DCAA5" }}>Listo — esperando a {opponent?.name} para la revancha</p></div>
          ) : (
            <Btn variant="success" onClick={() => send({ type: "player_ready" })}>Jugar de nuevo</Btn>
          )}
        </div>
        {ResetScoreControl}
        {isHost && <Btn variant="default" onClick={() => send({ type: "back_to_lobby" })} style={{ marginTop: 10 }}>Volver al lobby</Btn>}
      </div>
    );
  }
