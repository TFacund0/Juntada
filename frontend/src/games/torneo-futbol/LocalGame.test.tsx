...
async function goToConfigTab(user: ReturnType<typeof userEvent.setup>) {
  await user.click(screen.getByRole("button", { name: "Configuración" }));
}

async function prepareTwoPlayersNoGoals(user: ReturnType<typeof userEvent.setup>) {
...
describe("Torneo de Fútbol LocalGame", () => {
  test("renders the setup screen with 4 default players and 8 default teams", async () => {
  ...
  test("blocks starting when there are fewer teams than players", async () => {
    const user = userEvent.setup();
    render(<LocalGame />);
    await goToConfigTab(user);
    ...
    expect(screen.getByRole("button", { name: "Empezar torneo" })).toBeDisabled();
  });
  ...
  test("plays a full 2-player tournament to a champion", async () => {
    ...
    await prepareTwoPlayersNoGoals(user);
    await assignTeamsManually(user);

    await user.click(screen.getByRole("button", { name: "Empezar torneo" }));
