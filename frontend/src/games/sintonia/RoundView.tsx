...
export function RoundView({ room, me: _me, myPlayer: _myPlayer, myRole, wordReveal, isHost, send }: RoundViewProps) {
  ...
  const [setupSpectrumMode, setSetupSpectrumMode] = useState<"random" | "manual" | "same">("random");
      ...
      <div>
        ...
        <div style={S.card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {round?.lastSpectrum && (
              <button
                onClick={() => setSetupSpectrumMode("same")}
                style={{ ...S.btn(setupSpectrumMode === "same" ? "primary" : "ghost"), textAlign: "left" }}
              >
        ...
        <Btn variant="success" onClick={confirm} disabled={manualIncomplete}>
          Empezar ronda
        </Btn>
      </div>
    );
    ...
    return (
      <div>
        ...
        <div style={S.card}>
          ...
          <p style={{ ...S.muted, marginTop: 6 }}>
            {psychic?.name} gana lo mismo que sumaron los que adivinaron: +{round.psychicBonus ?? 0}
          </p>
        </div>
        <div style={S.card}>
