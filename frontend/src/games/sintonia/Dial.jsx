...
function pointAt(radius, valuePct) {
  const angle = (180 - (valuePct / 100) * 180) * (Math.PI / 180);
  return [CX + radius * Math.cos(angle), CY - radius * Math.sin(angle)];
}

function arcPath(radiusOuter, radiusInner, fromPct, toPct) {
...
export function Dial({ value, target = null, leftLabel, rightLabel, markers = [] }) {
  ...
  return (
    <div style={{ width: "100%", maxWidth: 320, margin: "0 auto" }}>
      <svg viewBox="0 0 300 175" width="100%" style={{ overflow: "visible" }}>
        ...
        {target != null && ZONES.slice().reverse().map((z, i) => {
        ...
        {target != null && ZONES.map((z, i) => {
          ...
          return [1, -1].map(sign => {
            ...
            return (
              <text key={`${i}-${sign}`} x={lx} y={ly} fill="#0f0c1d" fontSize={11} fontWeight={800} textAnchor="middle" dominantBaseline="middle">
                {z.points}
              </text>
            );
          });
        ...
        {markers.map((m, i) => {
          ...
          return (
            <g key={i}>
              ...
              {m.label && <text x={mx} y={my} fontSize={9} fontWeight={800} fill="#fff" textAnchor="middle" dominantBaseline="middle">{m.label}</text>}
            </g>
          );
        })}
