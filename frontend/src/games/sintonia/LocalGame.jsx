...
function Scoreboard({ players, history, totalScore }) {
...
export function LocalGame() {
  const [phase, setPhase] = useState("setup"); // setup|roundSetup|reveal|guessTurn|result
  const [players, setPlayers] = useState([{ id: 1, name: "Jugador 1" }, { id: 2, name: "Jugador 2" }, { id: 3, name: "Jugador 3" }]);
  const [newName, setNewName] = useState("");
  const [nameError, setNameError] = useState("");
  const [config, setConfig] = useState({ writtenClues: false });

  const [pool, setPool] = useState([]); // pares de la base sin usar en esta partida
  const [turnIdx, setTurnIdx] = useState(0);
  const [round, setRound] = useState(null); // { left, right, target, psychicId, psychicName, clue, guesses }
  ...
  const [setupSpectrumMode, setSetupSpectrumMode] = useState("random"); // random|manual
  ...
  const confirmRoundSetup = () => {
    ...
    let left, right, nextPool = pool;
    if (setupSpectrumMode === "manual" && setupManualLeft.trim() && setupManualRight.trim()) {
      left = setupManualLeft.trim(); right = setupManualRight.trim();
    } else {
      let source = pool;
      if (source.length === 0) source = shuffle(SPECTRUMS);
      [left, right] = source[0];
      nextPool = source.slice(1);
    }

    setRound({ left, right, target: randomTarget(), psychicId, psychicName: psychic.name, clue: null, guesses: {} });
    setPool(nextPool);
    setRevealed(false);
    setClueText("");
  ...
  const proceedToGuessing = () => {
    if (config.writtenClues) {
      ...
      setRound(r => ({ ...r, clue: clueText.trim() }));
    }
    setPhase("guessTurn");
  };
  ...
  const confirmCurrentGuess = () => {
    ...
    setRound(r => ({ ...r, guesses: nextGuesses }));
    ...
    setHistory(h => [...h, { left: round.left, right: round.right, target: round.target, psychicId: round.psychicId, psychicName: round.psychicName, guesses: nextGuesses, pointsByPlayer }]);
    setRound(r => ({ ...r, guesses: nextGuesses, pointsByPlayer }));
    setPhase("result");
  };
  ...
  if (phase === "setup") return (
    <div>
      ...
      <div style={S.card}>
        <Toggle label={config.writtenClues ? "Pistas escritas (se escriben en el dispositivo)" : "Pistas dichas en voz alta"} value={config.writtenClues} onChange={v => setConfig(c => ({ ...c, writtenClues: v }))} />
      </div>

      <Btn onClick={startGame} disabled={players.length < MIN_PLAYERS}>Iniciar partida</Btn>
      {players.length < MIN_PLAYERS && <p style={{ ...S.muted, textAlign: "center", marginTop: 8 }}>Necesitás mínimo {MIN_PLAYERS} jugadores</p>}
      ...
      {history.length > 0 && <div style={{ ...S.card, marginTop: 14 }}>
        ...
        {history.map((r, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(127,119,221,0.08)", fontSize: 13 }}>
            <span style={{ color: "#b8b0d4" }}>{r.psychicName} · {r.left} / {r.right}</span>
            <span style={{ color: (r.pointsByPlayer[r.psychicId] || 0) > 0 ? "#5DCAA5" : "#F09595" }}>psíquico +{r.pointsByPlayer[r.psychicId] || 0}</span>
          </div>
        ))}
      </div>}
    </div>
  ...
  if (phase === "roundSetup") {
    ...
    return (
      <div>
        ...
        <Btn variant="success" onClick={confirmRoundSetup} disabled={manualIncomplete}>Empezar ronda</Btn>
        <Btn variant="secondary" onClick={() => setPhase("setup")} style={{ marginTop: 10 }}>Volver a configuración</Btn>
      </div>
    );
  }
  ...
  if (phase === "reveal") {
    ...
    return (
      <div>
        ...
        <div
          style={{ ...S.card, textAlign: "center", cursor: "pointer", border: revealed ? "1px solid rgba(127,119,221,0.4)" : "1px solid rgba(255,255,255,0.08)", minHeight: 260, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", userSelect: "none" }}
        ...
        {revealed && !config.writtenClues && (
          <p style={{ ...S.muted, textAlign: "center", margin: "12px 0" }}>
            Pensá una pista (una palabra, una persona, lo que sea) que ubique ese punto entre "{round.left}" y "{round.right}" y decila en voz alta. No digas el objetivo directamente.
          </p>
        )}
        {revealed && config.writtenClues && (
          <div style={S.card}>
    ...
    return (
      <div>
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          <p style={S.muted}>Turno {guessIdx + 1} de {guessOrder.length} — pasále el dispositivo a esta persona</p>
        </div>
        {round.clue ? (
    ...
    return (
      <div>
        ...
        <div style={S.card}>
          ...
          {round.clue ? <p style={{ fontSize: 18, fontWeight: 700, margin: "4px 0 0" }}>"{round.clue}"</p> : <p style={{ ...S.muted, margin: "4px 0 0" }}>(dicha en voz alta)</p>}
          <p style={{ ...S.muted, marginTop: 6 }}>{round.psychicName} gana lo mismo que sumaron los que adivinaron: +{points[round.psychicId] || 0}</p>
        </div>
        <div style={S.card}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
          <Btn variant="success" onClick={goToRoundSetup}>Siguiente ronda</Btn>
          <Btn variant="secondary" onClick={() => setPhase("setup")}>Terminar partida</Btn>
        </div>
      </div>
    );
  }
