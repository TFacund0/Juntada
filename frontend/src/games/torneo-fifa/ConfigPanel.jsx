...
export function ConfigPanel({ room, updateConfig }) {
  const { players } = room;
  const { trackGoals, teams, assignments, seedOrder } = room.config;
  ...
  const removeTeam = (t) => updateConfig({ teams: teams.filter(x => x !== t) });

  const assignManually = (playerId, team) => {
  ...
  const spin = async (playerId, pool, finalTeam) => {
  ...
  const spinForPlayer = async (playerId) => {
    if (spinningId) return;
  ...
  const byeCount = order.length > 0 ? (() => {
    let p = 1; while (p < order.length) p *= 2; return p - order.length;
  })() : 0;

  return (
    <div>
      <div style={S.card}>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => updateConfig({ trackGoals: !trackGoals })}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
          {teams.map(t => (
            <span key={t} style={{ ...S.pill(true), cursor: "pointer", maxWidth: "100%" }} onClick={() => removeTeam(t)}>
              <span style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>🏳️ {t}</span> <span style={{ marginLeft: 4, opacity: 0.6, flexShrink: 0 }}>×</span>
            </span>
          ))}
        </div>
      ...
      <div style={{ ...S.cardHighlight, textAlign: "center" }}>
        <p style={{ fontSize: 13, color: "#9089c0", margin: "0 0 10px" }}>Asigná un equipo a cada jugador: con la ruleta o elegilo vos manualmente. Todos ven el resultado en vivo.</p>
        <Btn variant="success" onClick={runRouletteAll} disabled={!!spinningId || teams.length < players.length}>🎰 Girar la ruleta para todos</Btn>
      </div>
      ...
      {players.map(p => {
        ...
        return (
          <div key={p.id} style={S.card}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: (team && !isSpinning) ? 0 : 12 }}>
              ...
              {isSpinning ? (
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => spinForPlayer(p.id)} disabled={!!spinningId} style={{ ...S.btn("success", !!spinningId), width: "auto", padding: "8px 12px", fontSize: 13 }}>🎰</button>
            ...
            {isSpinning && (
              <div style={{ textAlign: "center", padding: "10px 8px", borderRadius: 10, background: "rgba(239,159,39,0.1)", border: "1px solid rgba(239,159,39,0.35)" }}>
            ...
            {!team && !isSpinning && manualPick === p.id && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
                {teams.filter(t => !usedTeams.has(t)).map(t => (
                  <button key={t} onClick={() => assignManually(p.id, t)} style={{ ...S.btn("default"), width: "auto", padding: "8px 14px", fontSize: 13, borderRadius: 8 }}>
                    {t}
                  </button>
                ))}
              </div>
      ...
      <div style={S.card}>
        ...
        {Array.from({ length: Math.ceil(order.length / 2) }, (_, pairIdx) => {
          ...
          const row = (p, idx) => (
            <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: 0, fontWeight: 700, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</p>
                <p style={{ margin: 0, fontSize: 11, color: "#7F77DD", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{assignments[p.id] || "sin equipo"}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                <button onClick={() => idx > 0 && moveSeed(idx, -1)} style={{ ...S.btn("default", idx === 0), width: 24, height: 18, padding: 0, borderRadius: 6, fontSize: 10, lineHeight: 1 }}>▲</button>
                <button onClick={() => idx < order.length - 1 && moveSeed(idx, 1)} style={{ ...S.btn("default", idx === order.length - 1), width: 24, height: 18, padding: 0, borderRadius: 6, fontSize: 10, lineHeight: 1 }}>▼</button>
              </div>
            </div>
          );
          ...
          return (
            <div key={pairIdx} style={{
              ...
              <p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", color: isBye ? "#6b6490" : "#7F77DD" }}>
        ...
        {byeCount > 0 && <p style={{ ...S.muted, fontSize: 12 }}>{byeCount} jugador{byeCount > 1 ? "es" : ""} pasa{byeCount > 1 ? "n" : ""} directo por no completar un cuadro parejo.</p>}
      </div>
    </div>
  );
}
