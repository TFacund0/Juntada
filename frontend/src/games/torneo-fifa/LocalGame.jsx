...
const DEFAULT_TEAMS = [
  ...
  "Real Madrid", "Barcelona", "Manchester City", "Boca Juniors",
];
...
export function LocalGame() {
  const [phase, setPhase] = useState("setup"); // setup|assign|seed|bracket|champion
  ...
  const [assignments, setAssignments] = useState({}); // playerId -> team
  const [manualPick, setManualPick] = useState(null); // playerId being assigned manually
  const [nameError, setNameError] = useState("");
  const [spinningId, setSpinningId] = useState(null); // playerId currently on the roulette
  const [spinLabel, setSpinLabel] = useState("");
  const [seedOrder, setSeedOrder] = useState([]); // array de playerId, orden de cruce
  const [rounds, setRounds] = useState(null);
  const [scoreInput, setScoreInput] = useState({ goalsA: "", goalsB: "" });
  const [editingMatch, setEditingMatch] = useState(null); // {roundIdx, matchIdx}
  ...
  const spinRoulette = async (playerId, pool, finalTeam) => {
  ...
  const spinRouletteForPlayer = async (playerId) => {
    if (spinningId) return;
  ...
  const assignTeamManually = (playerId, team) => {
    setAssignments(prev => ({ ...prev, [playerId]: team }));
    setManualPick(null);
  };

  const clearAssignment = (playerId) => {
  ...
  const confirmWinnerSimple = (roundIdx, matchIdx, winner) => {
    setRounds(prev => {
  ...
  const stats = () => {
    if (!rounds) return [];
    const tally = {};
    players.forEach(p => { tally[p.id] = { player: p, goalsFor: 0, goalsAgainst: 0, played: 0, won: 0 }; });
    rounds.forEach(round => round.forEach(m => {
      if (m.goalsA == null || m.goalsB == null || !m.a || !m.b) return;
      tally[m.a.id].goalsFor += m.goalsA; tally[m.a.id].goalsAgainst += m.goalsB; tally[m.a.id].played++;
      tally[m.b.id].goalsFor += m.goalsB; tally[m.b.id].goalsAgainst += m.goalsA; tally[m.b.id].played++;
    ...
    <div>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          ...
          <Btn variant="default" onClick={addPlayer} style={{ width: "auto", padding: "11px 18px" }}>Agregar</Btn>
        </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
          {teams.map(t => (
            <span key={t} style={{ ...S.pill(true), cursor: "pointer", maxWidth: "100%" }} onClick={() => setTeams(ts => ts.filter(x => x !== t))}>
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>🏳️ {t}</span> <span style={{ marginLeft: 4, opacity: 0.6, flexShrink: 0 }}>×</span>
            </span>
          ))}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <input style={{ ...S.input, flex: 1 }} placeholder="Agregar equipo..." value={newTeam} onChange={e => setNewTeam(e.target.value)} onKeyDown={e => { if (e.key === "Enter") addTeam(); }} />
          <Btn variant="default" onClick={addTeam} style={{ width: "auto", padding: "11px 18px" }}>Agregar</Btn>
        </div>
        {teams.length < players.length && <p style={{ fontSize: 12, color: "#F09595", marginTop: 8 }}>Necesitás al menos {players.length} equipos (uno por jugador)</p>}
      </div>

      <div style={S.card}>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => setTrackGoals(v => !v)}>
          <div style={S.toggle(trackGoals)}><div style={S.knob(trackGoals)} /></div>
      ...
      <Btn onClick={goToAssign} disabled={players.length < 2 || teams.length < players.length}>Continuar a sorteo de equipos</Btn>
  ...
  if (phase === "assign") return (
    <div>
      <div style={{ ...S.cardHighlight, textAlign: "center" }}>
        <p style={{ fontSize: 13, color: "#9089c0", margin: "0 0 10px" }}>Asigná un equipo a cada jugador: con la ruleta (aleatorio, uno por uno) o elegilo vos manualmente.</p>
        <Btn variant="success" onClick={runRouletteAll} disabled={!!spinningId}>🎰 Girar la ruleta para todos</Btn>
      </div>

      <div style={S.card}>
        <span style={S.label}>Equipos restantes ({teams.length - usedTeams.size} de {teams.length})</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {teams.map(t => {
            ...
            return (
              <span key={t} style={{
                display: "inline-flex", alignItems: "center", gap: 4, padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700,
                maxWidth: "100%", transition: "background 0.08s, border-color 0.08s, color 0.08s",
        ...
        return (
          <div key={p.id} style={S.card}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: (team && !isSpinning) ? 0 : 12 }}>
              <Avatar name={p.name} size={32} />
              <span style={{ fontWeight: 700, fontSize: 15, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</span>
              {isSpinning ? (
                ...
                <span style={{ ...S.pill(true), cursor: "pointer", maxWidth: "55%", flexShrink: 0 }} onClick={() => clearAssignment(p.id)}>
                  <span style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>🏳️ {team}</span> <span style={{ marginLeft: 4, opacity: 0.6, flexShrink: 0 }}>×</span>
                </span>
              ) : (
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button onClick={() => spinRouletteForPlayer(p.id)} disabled={!!spinningId} style={{ ...S.btn("success", !!spinningId), width: "auto", padding: "8px 12px", fontSize: 13 }}>🎰</button>
                  <Btn variant="default" onClick={() => setManualPick(manualPick === p.id ? null : p.id)} disabled={!!spinningId} style={{ width: "auto", padding: "8px 14px", fontSize: 13 }}>
                    Elegir equipo
                  </Btn>
                </div>
              )}
            ...
            {isSpinning && (
              <div style={{
                ...
                background: "rgba(239,159,39,0.1)", border: "1px solid rgba(239,159,39,0.35)",
            ...
            {!team && !isSpinning && manualPick === p.id && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
                {teams.filter(t => !usedTeams.has(t)).map(t => (
                  <button key={t} onClick={() => assignTeamManually(p.id, t)} style={{ ...S.btn("default"), width: "auto", padding: "8px 14px", fontSize: 13, borderRadius: 8 }}>
                    {t}
                  </button>
                ))}
              </div>
      ...
      <Btn onClick={goToSeed} disabled={!allAssigned || !!spinningId} variant="success" style={{ marginTop: 8 }}>Continuar a armar los cruces</Btn>
      <Btn variant="default" onClick={() => setPhase("setup")} style={{ marginTop: 10 }}>Volver</Btn>
    </div>
  );
      ...
      <div>
        <div style={{ ...S.cardHighlight, textAlign: "center" }}>
          <p style={{ fontSize: 13, color: "#9089c0", margin: "0 0 10px" }}>
            Este es el orden de cruce de la primera ronda: cada par de la lista se enfrenta entre sí. Usá las flechas para armarlo a mano, o sorteá el orden al azar.
          </p>
          <Btn variant="success" onClick={randomizeSeed}>🎲 Sortear cruces al azar</Btn>
        </div>

        {byeCount > 0 && (
          <p style={{ ...S.muted, textAlign: "center", marginBottom: 14 }}>
            {seedOrder.length} jugadores no completan un cuadro parejo: {byeCount === 1 ? "el último de la lista" : `los últimos ${byeCount} de la lista`} pasa{byeCount === 1 ? "" : "n"} directo a la siguiente ronda (bye).
          </p>
        )}
        ...
        {Array.from({ length: Math.ceil(seedOrder.length / 2) }, (_, pairIdx) => {
          ...
          const row = (p, idx) => (
            <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ margin: 0, fontWeight: 700, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</p>
                <p style={{ margin: 0, fontSize: 11, color: "#7F77DD", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{assignments[p.id]}</p>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                <button onClick={() => idx > 0 && moveSeed(idx, -1)} style={{ ...S.btn("default", idx === 0), width: 26, height: 20, padding: 0, borderRadius: 6, fontSize: 11, lineHeight: 1 }}>▲</button>
                <button onClick={() => idx < seedOrder.length - 1 && moveSeed(idx, 1)} style={{ ...S.btn("default", idx === seedOrder.length - 1), width: 26, height: 20, padding: 0, borderRadius: 6, fontSize: 11, lineHeight: 1 }}>▼</button>
              </div>
            </div>
          );
          ...
          return (
            <div key={pairIdx} style={{
              ...
              <p style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", color: isBye ? "#6b6490" : "#7F77DD" }}>
        ...
        <Btn onClick={startBracket} variant="success">Confirmar cruces y empezar torneo</Btn>
        <Btn variant="default" onClick={() => setPhase("assign")} style={{ marginTop: 10 }}>Volver</Btn>
      </div>
    );
  }
  ...
  if (phase === "bracket" || phase === "champion") {
        ...
        <div>
            ...
            <div style={S.card}>
              ...
              {topScorer && topScorer.goalsFor > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 13 }}>Máximo goleador: <strong style={{ color: "#5DCAA5" }}>{topScorer.player.name}</strong> ({topScorer.goalsFor} goles)</span>
                </div>
              )}
              {leakiest && leakiest.goalsAgainst > 0 && (
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 20 }}>🥅</span>
                  <span style={{ fontSize: 13 }}>Valla más goleada: <strong style={{ color: "#F09595" }}>{leakiest.player.name}</strong> ({leakiest.goalsAgainst} recibidos)</span>
                </div>
              )}
            </div>
          )}
          ...
          {trackGoals && (
            <div style={S.card}>
              ...
              <div style={{ display: "flex", fontSize: 11, color: "#6b6490", padding: "0 0 8px", borderBottom: "1px solid rgba(127,119,221,0.15)" }}>
              ...
              {s.map(row => (
                <div key={row.player.id} style={{ display: "flex", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(127,119,221,0.08)", fontSize: 13 }}>
                  <span style={{ flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {row.player.id === champion.id && <span title="Campeón" style={{ flexShrink: 0 }}>🏆</span>}
                  </span>
                  ...
                  <span style={{ width: 40, flexShrink: 0, textAlign: "center", fontWeight: 700 }}>{row.goalsFor - row.goalsAgainst >= 0 ? "+" : ""}{row.goalsFor - row.goalsAgainst}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={S.card}>
            ...
            {rounds.map((round, ri) => (
              <div key={ri} style={{ marginBottom: 10 }}>
                <p style={{ fontSize: 11, color: "#7F77DD", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 6px" }}>{roundNames[ri]}</p>
                {round.map((m, mi) => (
                  <div key={mi} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", fontSize: 13 }}>
                    <span style={{ flex: 1, minWidth: 0, textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: m.winner?.id === m.a?.id ? "#5DCAA5" : "#9089c0" }}>{m.a ? m.a.name : "—"}</span>
                    <span style={{ flexShrink: 0, color: "#6b6490", fontSize: 12 }}>{m.goalsA != null ? `${m.goalsA} - ${m.goalsB}` : "vs"}</span>
                    <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: m.winner?.id === m.b?.id ? "#5DCAA5" : "#9089c0" }}>{m.b ? m.b.name : "—"}</span>
                  </div>
                ))}
              </div>
            ))}
    ...
    const sideStyle = (side, m) => ({
      display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: 1,
      opacity: m.winner && m.winner.id !== side?.id ? 0.45 : 1,
    });
    const nameStyle = (side, m) => ({
      margin: 0, fontWeight: 700, fontSize: 13, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
      color: m.winner?.id === side?.id ? "#5DCAA5" : "#e8e4f0",
    });
    const teamStyle = { margin: 0, fontSize: 11, color: "#7F77DD", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
    ...
    return (
      <div>
                ...
                <div key={mi} style={{ ...S.cardHighlight, marginBottom: 10, padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={sideStyle(m.a, m)}>
                      {m.a ? <Avatar name={m.a.name} size={30} /> : <div style={{ width: 30, height: 30, borderRadius: "50%", border: "1px dashed rgba(127,119,221,0.3)", flexShrink: 0 }} />}
                    ...
                    <div style={{ flexShrink: 0, textAlign: "center", minWidth: 46 }}>
                      {m.goalsA != null ? (
                        <span style={{ fontSize: 16, fontWeight: 800, color: "#AFA9EC" }}>{m.goalsA} - {m.goalsB}</span>
                      ) : (
                        <span style={{ fontSize: 11, fontWeight: 700, color: "#6b6490" }}>vs</span>
                      )}
                    </div>

                    <div style={{ ...sideStyle(m.b, m), flexDirection: "row-reverse", textAlign: "right" }}>
                      {m.b ? <Avatar name={m.b.name} size={30} /> : <div style={{ width: 30, height: 30, borderRadius: "50%", border: "1px dashed rgba(127,119,221,0.3)", flexShrink: 0 }} />}
                      <div style={{ minWidth: 0 }}>
                  ...
                  {playable && !editing && (
                    <Btn variant="default" onClick={() => openMatch(ri, mi)} style={{ marginTop: 12 }}>Cargar resultado</Btn>
                  )}

                  {playable && editing && !trackGoals && (
                    <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                      <Btn variant="success" onClick={() => confirmWinnerSimple(ri, mi, m.a)} style={{ fontSize: 13 }}>Ganó {m.a.name}</Btn>
                      <Btn variant="success" onClick={() => confirmWinnerSimple(ri, mi, m.b)} style={{ fontSize: 13 }}>Ganó {m.b.name}</Btn>
                    </div>
                  )}
                  ...
                  {playable && editing && trackGoals && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                        <span style={{ fontSize: 13, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.a.name}</span>
                        <input type="number" min="0" style={{ ...S.input, width: 60, flexShrink: 0, textAlign: "center" }} value={scoreInput.goalsA} onChange={e => setScoreInput(s => ({ ...s, goalsA: e.target.value }))} />
                      </div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
                        <span style={{ fontSize: 13, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.b.name}</span>
                        <input type="number" min="0" style={{ ...S.input, width: 60, flexShrink: 0, textAlign: "center" }} value={scoreInput.goalsB} onChange={e => setScoreInput(s => ({ ...s, goalsB: e.target.value }))} />
                      </div>
                      <Btn variant="success" onClick={() => confirmScore(ri, mi)}>Confirmar resultado</Btn>
                    </div>
                  )}

                  {decided && !playable && m.a && m.b && (
                    <p style={{ ...S.muted, marginTop: 8, textAlign: "center" }}>Ganó {m.winner.name} · {m.winner.team}</p>
                  )}
                </div>
              );
            })}
        ...
        <Btn variant="default" onClick={() => setPhase("setup")}>Cancelar torneo</Btn>
      </div>
    );
  }
