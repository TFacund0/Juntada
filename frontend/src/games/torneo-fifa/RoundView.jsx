...
export function RoundView({ room, myPlayer, isHost, send }) {
  const { rounds, trackGoals } = room.round;
  ...
  const confirmWinnerSimple = (roundIdx, matchIdx, side) => {
  ...
  const stats = () => {
    const tally = {};
    room.players.forEach(p => { tally[p.id] = { player: p, goalsFor: 0, goalsAgainst: 0, played: 0, won: 0 }; });
    ...
    <div>
      {!isHost && (
        <div style={{ ...S.cardHighlight, textAlign: "center", marginBottom: 16 }}>
          <p style={{ fontSize: 13, color: "#9089c0", margin: 0 }}>El anfitrión va cargando los resultados a medida que se juegan los partidos.</p>
        </div>
      )}
      {rounds.map((round, ri) => (
        <div key={ri} style={{ marginBottom: 18 }}>
          ...
          {round.map((m, mi) => {
            ...
            return (
              <div key={mi} style={{ ...S.cardHighlight, marginBottom: 10, padding: "14px 16px", border: involvesMe && playable ? "1px solid rgba(93,202,165,0.5)" : undefined }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {isHost && playable && editing && !trackGoals && (
                  <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                    <Btn variant="success" onClick={() => confirmWinnerSimple(ri, mi, "a")} style={{ fontSize: 13 }}>Ganó {m.a.name}</Btn>
                    <Btn variant="success" onClick={() => confirmWinnerSimple(ri, mi, "b")} style={{ fontSize: 13 }}>Ganó {m.b.name}</Btn>
                  </div>
                )}
                ...
                {!isHost && playable && (
                  <p style={{ ...S.muted, marginTop: 8, textAlign: "center" }}>{involvesMe ? "Es tu partido — esperá a que el anfitrión cargue el resultado." : "Esperando resultado..."}</p>
                )}
